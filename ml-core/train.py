...
def load_data(file_path):
    ...
    X = []
    for item in data:
        features = list(item['board'])
        features.append(item['move'])
        X.append(features)
        
    X = np.array(X, dtype=np.float32)
    y = np.array([item['result'] for item in data], dtype=np.float32).reshape(-1, 1)
...
def train():
    ...
    try:
        X_all, y_all = load_data(INPUT_FILE_PATH)
