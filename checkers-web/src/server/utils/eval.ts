...
import { type ShallowCandidate, buildShallowCandidates, filterCandidatesByDelta, sortCandidatesByShallowInPlace } from './prune'

type PlayerToMove = -1 | 1

type Bounds = { alpha?: number; beta?: number; useAlphaBeta?: boolean }
...
export async function evaluateBoardShallow(board: BoardPosition, move: Player): Promise<number> {
  ...
  async function evaluateBoardDeeplyWithBounds(
    ...
    depth: number,
    bounds: Bounds = { useAlphaBeta: true },
  ): Promise<number> {
    if (depth === 0) {
    ...
    // TODO: is it ok? Maybe it SHOULD really be 0? XD
    if (depth === 1) {
  ...
  export async function evaluateBoardDeeply(
    ...
    currentPlayer: Player,
    depth: number,
    bounds: Bounds = { useAlphaBeta: true, alpha: -Infinity, beta: Infinity },
  ): Promise<number> {
