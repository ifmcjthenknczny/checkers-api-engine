...
const QuerySchema = z.object({
  ...
  modelLevel: z.coerce
    .number()
    .refine((n) => ([0, ...MODEL_LEVELS] as readonly number[]).includes(n), {
      message: `modelLevel must be one of: ${MODEL_LEVELS.join(', ')}`,
    }),
...
export default defineEventHandler((event) => {
  ...
  runTask('scrape', {
    payload: { games, modelLevel: modelLevel as ScrapeModelLevel, random, depth },
  }).catch((error) => console.error('[scrape] Fatal error:', error))
