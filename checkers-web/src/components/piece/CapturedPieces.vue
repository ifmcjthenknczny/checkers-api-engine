<template>
  <section class="captured-pieces">
    <div
      ...
      :key="'t-q-' + n"
      :class="['mini-piece', 'mini-piece--queen', `mini-piece--${props.pieceColor}`]"
    />
    <div
      v-for="n in Array.from({ length: capturedNormalPiecesCount })"
      :key="'t-p-' + n"
      :class="['mini-piece', `mini-piece--${props.pieceColor}`]"
    />
  </section>
</template>
...
const capturedQueensCount = computed(() => {
  const currentQueens = getPiecesOfColor(board.value, props.pieceColor).filter(({ piece }) =>
    isQueen(piece),
  ).length
